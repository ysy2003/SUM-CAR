"""
Cross-task evaluation: Test math-trained memory (GSM8K) on FinQA task.
"""
import os
import json
import fire
...
def eval_finqa(model, tokenizer, max_samples=99999, use_cot=True):
    ...
    for ex in tqdm(ds_list, desc="  FinQA", unit="sample"):
        ...
        try:
            ...
            # Check correctness (use tolerant matching for financial data)
            ...
            predictions.append({
                'context': ctx[:200] + '...' if len(ctx) > 200 else ctx,  # Truncate for storage
                'question': q,
                'gold': gold,
                'pred': pred,
                'correct': is_correct
...
def main(base_model='meta-llama/Meta-Llama-3-8B-Instruct',
         ...
         out='noLoRA/math_only/acc_72%/eval/finqa_cross_results.json',
         k_top=64,
         alpha=1.0,
         use_cot=True,
         use_fp16=False,
    ...
    """
    Cross-task evaluation: Test math-trained memory on FinQA.

    Args:
        base_model: Base model name
        merged_dir: Directory with merged memory.pt (from math training)
        out: Output JSON path
        k_top: Top-k for memory retrieval
        alpha: Alpha parameter
        use_cot: Use Chain-of-Thought prompting
        use_fp16: Use FP16 precision
        mode: '100' for first 100 only, 'full' for full dataset
        memory_position: 'embedding' or 'middle' (after layer 16)
    """
    max_samples = 100 if str(mode) == '100' else 99999

    print(f"=== Cross-Task Evaluation: Math Memory -> FinQA ===")
    ...
    # Load model with math-trained memory
    print("Loading math-trained memory model...")
    ...
    print("\n" + "="*50)
    print("FinQA Evaluation (using math-trained memory)")
    print("="*50)
    ...
    print("Summary")
    print("="*50)
    print(f"Source: Math-trained memory (GSM8K)")
