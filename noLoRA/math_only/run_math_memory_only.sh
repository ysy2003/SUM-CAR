...
set -e
# Change to SUM-CAR root directory (parent of noLoRA/math_only)
cd "$(dirname "$0")/../.."
...
mkdir -p noLoRA/math_only
mkdir -p noLoRA/math_only/eval
...
if [ ! -f "noLoRA/math_only/patch_gsm8k.json" ]; then
    echo "Training memory layer on GSM8K..."
    TRAIN_CMD="python -m sumcar.cli.train_task --task gsm8k --config noLoRA/math_only/train_math_memory_only.yaml"
    [ -n "$MAX_EXAMPLES" ] && TRAIN_CMD="$TRAIN_CMD --max_examples $MAX_EXAMPLES"
    [ -n "$EPOCHS" ] && TRAIN_CMD="$TRAIN_CMD --epochs $EPOCHS"
    eval $TRAIN_CMD
    echo ""
    echo "+ Training complete. Patch saved to: noLoRA/math_only/patch_gsm8k.json"
else
    echo "+ Patch already exists: noLoRA/math_only/patch_gsm8k.json"
fi
...
if [ ! -f "$MERGED_DIR/memory.pt" ]; then
    ...
    python -m sumcar.cli.merge_patches \
        ...
        --patches noLoRA/math_only/patch_gsm8k.json \
        --out "$MERGED_DIR" \
        --num_slots 8192 \
        --k_top 64 \
        --alpha 1.0 \
...
OUT_FILE="noLoRA/math_only/eval/math_results_cot.json"
...
echo ""
echo "Results:"
echo "  - Patch: noLoRA/math_only/patch_gsm8k.json"
